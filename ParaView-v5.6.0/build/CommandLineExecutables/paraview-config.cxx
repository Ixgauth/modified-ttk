  ...
  if (flag == "--libs")
    ...
    for (vtkModuleNames::const_iterator i = neededModules.begin(); i != neededModules.end(); ++i)
      ...
      if (usePython && modulesWithPython.count(module))
        {
        std::cout << " -l" << module << "Python36D-pv" PARAVIEW_VERSION;
        }
    ...
    {
    std::cout << " -I/usr/local/include/paraview-5.6" << std::endl;
    }
  else if (flag == "--list")
    {
    ...
    {
    std::cout << " -l/usr/lib/x86_64-linux-gnu/libpython3.6m.so" << std::endl;
    }
  else
    {
    die("Usage: " << argv[0] << " " << flagUsage);
