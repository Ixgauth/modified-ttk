/****************************************************************************
** Meta object code from reading C++ file 'pqChartSelectionReaction.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../Qt/ApplicationComponents/pqChartSelectionReaction.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pqChartSelectionReaction.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_pqChartSelectionReaction_t {
    QByteArrayData data[6];
    char stringdata0[88];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_pqChartSelectionReaction_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_pqChartSelectionReaction_t qt_meta_stringdata_pqChartSelectionReaction = {
    {
QT_MOC_LITERAL(0, 0, 24), // "pqChartSelectionReaction"
QT_MOC_LITERAL(1, 25, 9), // "triggered"
QT_MOC_LITERAL(2, 35, 0), // ""
QT_MOC_LITERAL(3, 36, 13), // "stopSelection"
QT_MOC_LITERAL(4, 50, 16), // "modifiersChanged"
QT_MOC_LITERAL(5, 67, 20) // "getSelectionModifier"

    },
    "pqChartSelectionReaction\0triggered\0\0"
    "stopSelection\0modifiersChanged\0"
    "getSelectionModifier"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_pqChartSelectionReaction[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x09 /* Protected */,
       3,    0,   37,    2, 0x09 /* Protected */,
       4,    0,   38,    2, 0x09 /* Protected */,
       5,    0,   39,    2, 0x09 /* Protected */,

 // slots: parameters
    QMetaType::Void, QMetaType::Bool,    2,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Int,

       0        // eod
};

void pqChartSelectionReaction::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        pqChartSelectionReaction *_t = static_cast<pqChartSelectionReaction *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->triggered((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: _t->stopSelection(); break;
        case 2: _t->modifiersChanged(); break;
        case 3: { int _r = _t->getSelectionModifier();
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = std::move(_r); }  break;
        default: ;
        }
    }
}

const QMetaObject pqChartSelectionReaction::staticMetaObject = {
    { &pqSelectionReaction::staticMetaObject, qt_meta_stringdata_pqChartSelectionReaction.data,
      qt_meta_data_pqChartSelectionReaction,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *pqChartSelectionReaction::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *pqChartSelectionReaction::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_pqChartSelectionReaction.stringdata0))
        return static_cast<void*>(this);
    return pqSelectionReaction::qt_metacast(_clname);
}

int pqChartSelectionReaction::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = pqSelectionReaction::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
