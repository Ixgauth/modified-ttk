/****************************************************************************
** Meta object code from reading C++ file 'pqUndoRedoReaction.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../Qt/ApplicationComponents/pqUndoRedoReaction.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pqUndoRedoReaction.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_pqUndoRedoReaction_t {
    QByteArrayData data[7];
    char stringdata0[68];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_pqUndoRedoReaction_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_pqUndoRedoReaction_t qt_meta_stringdata_pqUndoRedoReaction = {
    {
QT_MOC_LITERAL(0, 0, 18), // "pqUndoRedoReaction"
QT_MOC_LITERAL(1, 19, 6), // "enable"
QT_MOC_LITERAL(2, 26, 0), // ""
QT_MOC_LITERAL(3, 27, 8), // "setLabel"
QT_MOC_LITERAL(4, 36, 5), // "label"
QT_MOC_LITERAL(5, 42, 12), // "setUndoStack"
QT_MOC_LITERAL(6, 55, 12) // "pqUndoStack*"

    },
    "pqUndoRedoReaction\0enable\0\0setLabel\0"
    "label\0setUndoStack\0pqUndoStack*"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_pqUndoRedoReaction[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x09 /* Protected */,
       3,    1,   32,    2, 0x09 /* Protected */,
       5,    1,   35,    2, 0x09 /* Protected */,

 // slots: parameters
    QMetaType::Void, QMetaType::Bool,    2,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, 0x80000000 | 6,    2,

       0        // eod
};

void pqUndoRedoReaction::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        pqUndoRedoReaction *_t = static_cast<pqUndoRedoReaction *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->enable((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: _t->setLabel((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 2: _t->setUndoStack((*reinterpret_cast< pqUndoStack*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObject pqUndoRedoReaction::staticMetaObject = {
    { &pqReaction::staticMetaObject, qt_meta_stringdata_pqUndoRedoReaction.data,
      qt_meta_data_pqUndoRedoReaction,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *pqUndoRedoReaction::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *pqUndoRedoReaction::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_pqUndoRedoReaction.stringdata0))
        return static_cast<void*>(this);
    return pqReaction::qt_metacast(_clname);
}

int pqUndoRedoReaction::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = pqReaction::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
