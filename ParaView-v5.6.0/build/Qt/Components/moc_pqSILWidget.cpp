/****************************************************************************
** Meta object code from reading C++ file 'pqSILWidget.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../Qt/Components/pqSILWidget.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pqSILWidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_pqSILWidget_t {
    QByteArrayData data[7];
    char stringdata0[97];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_pqSILWidget_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_pqSILWidget_t qt_meta_stringdata_pqSILWidget = {
    {
QT_MOC_LITERAL(0, 0, 11), // "pqSILWidget"
QT_MOC_LITERAL(1, 12, 12), // "onModelReset"
QT_MOC_LITERAL(2, 25, 0), // ""
QT_MOC_LITERAL(3, 26, 19), // "checkSelectedBlocks"
QT_MOC_LITERAL(4, 46, 21), // "uncheckSelectedBlocks"
QT_MOC_LITERAL(5, 68, 20), // "toggleSelectedBlocks"
QT_MOC_LITERAL(6, 89, 7) // "checked"

    },
    "pqSILWidget\0onModelReset\0\0checkSelectedBlocks\0"
    "uncheckSelectedBlocks\0toggleSelectedBlocks\0"
    "checked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_pqSILWidget[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   39,    2, 0x09 /* Protected */,
       3,    0,   40,    2, 0x09 /* Protected */,
       4,    0,   41,    2, 0x09 /* Protected */,
       5,    1,   42,    2, 0x09 /* Protected */,
       5,    0,   45,    2, 0x29 /* Protected | MethodCloned */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Bool,    6,
    QMetaType::Void,

       0        // eod
};

void pqSILWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        pqSILWidget *_t = static_cast<pqSILWidget *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->onModelReset(); break;
        case 1: _t->checkSelectedBlocks(); break;
        case 2: _t->uncheckSelectedBlocks(); break;
        case 3: _t->toggleSelectedBlocks((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 4: _t->toggleSelectedBlocks(); break;
        default: ;
        }
    }
}

const QMetaObject pqSILWidget::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_pqSILWidget.data,
      qt_meta_data_pqSILWidget,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *pqSILWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *pqSILWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_pqSILWidget.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int pqSILWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
