// ClientServer wrapper for vtkExtractSurface object
//
...
vtkObjectBase *vtkExtractSurfaceClientServerNewCommand(void* /*ctx*/)
{
  return vtkExtractSurface::New();
}


int VTK_EXPORT vtkExtractSurfaceCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkExtractSurface *op = vtkExtractSurface::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkExtractSurface.  "
  ...
  if (!strcmp("SetHoleFilling",method) && msg.GetNumberOfArguments(0) == 3)
    {
    bool   temp0;
    if(msg.GetArgument(0, 2, &temp0))
      {
  ...
  if (!strcmp("GetHoleFilling",method) && msg.GetNumberOfArguments(0) == 2)
    {
    bool   temp20;
      {
      temp20 = (op)->GetHoleFilling();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("HoleFillingOn",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
      op->HoleFillingOn();
      return 1;
  ...
  if (!strcmp("HoleFillingOff",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
      op->HoleFillingOff();
      return 1;
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkExtractSurface, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkExtractSurface_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkExtractSurface", vtkExtractSurfaceClientServerNewCommand);
    csi->AddCommandFunction("vtkExtractSurface", vtkExtractSurfaceCommand);
    }
}
