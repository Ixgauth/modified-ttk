// ClientServer wrapper for vtkFlyingEdgesPlaneCutter object
//
...
vtkObjectBase *vtkFlyingEdgesPlaneCutterClientServerNewCommand(void* /*ctx*/)
{
  return vtkFlyingEdgesPlaneCutter::New();
}


int VTK_EXPORT vtkFlyingEdgesPlaneCutterCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkFlyingEdgesPlaneCutter *op = vtkFlyingEdgesPlaneCutter::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkFlyingEdgesPlaneCutter.  "
  ...
  if (!strcmp("New",method) && msg.GetNumberOfArguments(0) == 2)
    {
    vtkFlyingEdgesPlaneCutter  *temp20;
      {
      temp20 = (op)->New();
      resultStream.Reset();
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkFlyingEdgesPlaneCutter, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkFlyingEdgesPlaneCutter_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkFlyingEdgesPlaneCutter", vtkFlyingEdgesPlaneCutterClientServerNewCommand);
    csi->AddCommandFunction("vtkFlyingEdgesPlaneCutter", vtkFlyingEdgesPlaneCutterCommand);
    }
}
