// ClientServer wrapper for vtkmProbe object
//
...
vtkObjectBase *vtkmProbeClientServerNewCommand(void* /*ctx*/)
{
  return vtkmProbe::New();
}


int VTK_EXPORT vtkmProbeCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkmProbe *op = vtkmProbe::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkmProbe.  "
  ...
  if (!strcmp("SetValidPointMaskArrayName",method) && msg.GetNumberOfArguments(0) == 3)
    ...
    if(msg.GetArgument(0, 2, &temp0))
      {
      op->SetValidPointMaskArrayName(vtkStdString(temp0));
      return 1;
      }
    }
  ...
  if (!strcmp("SetValidCellMaskArrayName",method) && msg.GetNumberOfArguments(0) == 3)
    {
    char    *  temp0;
    if(msg.GetArgument(0, 2, &temp0))
      {
      op->SetValidCellMaskArrayName(vtkStdString(temp0));
      return 1;
      }
    }
  if (!strcmp("GetValidCellMaskArrayName",method) && msg.GetNumberOfArguments(0) == 2)
    {
    std::string   temp20;
      {
      temp20 = (op)->GetValidCellMaskArrayName();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkmProbe, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkmProbe_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkmProbe", vtkmProbeClientServerNewCommand);
    csi->AddCommandFunction("vtkmProbe", vtkmProbeCommand);
    }
}
