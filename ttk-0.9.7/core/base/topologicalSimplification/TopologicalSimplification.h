...
//gonna do line by line fore the next two methods 
template <typename dataType>
int ttk::TopologicalSimplification::addPerturbation(dataType* scalars, SimplexId* 
offsets) const{
  dataType epsilon{};

  //dbl_dig and flt_dig are the minimum number of significant digits for each of those data types
  //so epsilon will be the maximum error value possible for that data type due to conversion between that type to text and back to the type
  ...
  //create a new vector of tuples where each tuple's 0 spot houses a scalar each 1 spot a offset and each 2 spot simply the index
...
//we will also put all input offsets into the offset vector 
  ...
  for(SimplexId i=0; i<vertexNumber_; ++i){
    ...
    // there will only be one meaningful run through
    for(int j=0; j<2; ++j){
      
      //if we are on the first iteration for a given point we will say that we don't want increasing order, if we are later we want increasig order
