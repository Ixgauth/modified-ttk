...
int ttkBettiNumbers::doIt(vector<vtkDataSet *> &inputs, vector<vtkDataSet *> &outputs){
  ...
  bettiNumbers_.setupTriangulation(triangulation);
  bettiNumbers_.setWrapper(this);
  ...
  if(!outputScalarField_){
    switch(inputScalarField->GetDataType()){
      ...
      stringstream msg;
      msg << "[ttkBettiNumbers] Unsupported data type :(" << endl;
  ...
  bettiNumbers_.setInputDataPointer(inputScalarField->GetVoidPointer(0));
  bettiNumbers_.setOutputDataPointer(outputScalarField_->GetVoidPointer(0));
  switch(inputScalarField->GetDataType()){
    ttkTemplateMacro(bettiNumbers_.execute<VTK_TT>(SomeIntegerArgument));
  }
  
  {
    stringstream msg;
    msg << "[ttkBettiNumbers] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
