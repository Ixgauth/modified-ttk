...
//this is similar to what has been shown in the online tutorial, what we are basically doing is simply defining the information that will be placed in the output ports
//in this case, that takes the form of us saying that each of the four outputs of the filter will be vtkTables which makes sense given the nature of what we are trying to do
//when called we will simply look at one of the four ports and define its output then return
...
//based on what I've learned from watching the 'implement new modules' video, it seems this is the base of filter as a whole
//which makes sense but 'do it' wasn't my favorite nomanclature for that
int ttkPersistenceCurve::doIt(vtkDataSet *input,
  ...
  //so this is the first time that we call persistenceCurve_ which will call the PersistenceCurve class and use the methods defined there
