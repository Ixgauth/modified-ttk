...
: public vtkDataSetAlgorithm, public ttk::Wrapper{
  ...
  public:
    ...
    int setInputVerticesNumbers(double *input);
    int setCriticalPointsToRemove(double *input);

    //simple setter for number of critical points in image
    int setNumberOfCriticalPoints(int number)
    {
        numberOfCriticalPoints_=number;
        return 0;
    }

    int setNumberOfCriticalPointsToRemove(int number)
    {
        numberOfCriticalPointsToRemove_ = number;
        return 0;
    }

    //sets whether we are working with min or max as the remaining critical point
    //0 is min
    // 3 is max
    // 1 and 2 should not be input as they will result in errors as they are saddle points 
    int setCriticalType(int criticalType)
    {
        inputCriticalType_ = criticalType;
        return 0;
    }
