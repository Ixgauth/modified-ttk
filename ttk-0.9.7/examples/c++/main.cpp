...
int main(int argc, char **argv) {
  ...
  //this is the  set of x,y,z coordinates for the mesh which we are about to load
  std::vector<float> pointSet;

  //this is the set of triangles for the mesh
  //if you remember from graphics how the triangles were used to create non-standard mesh shapes this is the idea here
  std::vector<long long int> triangleSet;

  //triangulation here is not used as it would be in other systems (determining a new point from three existing ones)
  //but instead the class wil make the triangular mesh from the input data/points
  ttk::Triangulation triangulation;

  // load the input
  load(inputFilePath, pointSet, triangleSet);

  //what this will do is we will import into the triangulation object the points we are trying to use for the mesh
  triangulation.setInputPoints(pointSet.size()/3, pointSet.data());
  //we will do the same here only with the triangles that make up the mesh
