...
int main(int argc, char **argv){
  ...
  //vtkSmartPointer seems to be used to call other vtk clases and instantiate an object of that type
  //in this case we are calling on the data loader to intake the VTU file which we input the file name for above
  vtkSmartPointer<vtkXMLUnstructuredGridReader> reader = 
    vtkSmartPointer<vtkXMLUnstructuredGridReader>::New();
  reader->SetFileName(inputFilePath.data());
 
  // 2. computing the persistence curve
  //create the persistence curve object
  //setInputConnection is a vtk function that will define what the input port for a given function
  //in this case, we are choosing the output of reader which took in the data from the image we gave it and that is what will be the input for the curve object
  ...
  //basically same as above, instantiate the diagram and then set its input port as the output for reader
